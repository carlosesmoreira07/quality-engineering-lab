...
Este laboratório demonstra, de forma isolada e reversível, que controles já existentes detectam regressões relevantes. Cada execução conta a mesma história: baseline saudável **GREEN** → variante controlada **RED** → baseline restaurada **GREEN**. O SUT padrão não é alterado.

| Experimento | Risco | Regressão temporária | Controle existente |
|---|---|---|---|
| EXP-001 | RISK-002 | preço apresentado como `$36.00` no lugar de `$35.00` na página do produto | `product-cart.spec.ts` |
| EXP-002 | RISK-018 | atraso local de 1.250 ms na página do produto | cenário k6 `smoke.js`, com `p(95)<1000` inalterado |
| EXP-003 | RISK-016 | resposta local `200` para cancelamento anônimo que deveria retornar `401` | `authorization-boundaries.spec.ts` |
...
É possível apontar para outra baseline saudável com `BASE_URL`. Os comandos locais retornam sucesso somente quando observam GREEN → RED → GREEN. Evidências concisas são gravadas em `experiments/results/` e não são versionadas.
...
A skill `qe-impact-analysis` foi aplicada consultivamente ao EXP-001. Ela relacionou a alteração observável de preço ao RISK-002 e confirmou que a asserção existente no fluxo de produto/carrinho é um controle adequado. Nenhum teste, threshold, Jira ou Quality Gate foi modificado pela análise.
