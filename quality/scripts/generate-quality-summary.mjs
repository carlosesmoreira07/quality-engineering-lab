...
function parseRiskCatalog(strategy) {
  ...
  for (const line of strategy.split(/\r?\n/)) {
    ...
    if (!id || !name || !severity || !priorityValues[severity]) {
      throw new Error(`Linha de risco inválida em ${riskStrategyPath}: ${line}`);
    }
    ...
    risks[id] = {
      ...
      probability,
      severity,
      priority: priorityValues[severity],
      control
    };
  }
...
function riskPills(risks, { executive = false } = {}) {
  return risks.map((riskId) => {
    const risk = riskCatalog[riskId];
    if (!risk) throw new Error(`Risco ${riskId} não existe na matriz ${riskStrategyPath}`);
...
.risk-pill{display:inline-grid;gap:.4mm;align-items:start;max-width:90mm;padding:1.2mm 2.2mm;border-radius:2mm}.risk-pill strong{font-size:7pt;line-height:1.2}.risk-pill small{color:#5d6b62;font-size:5.8pt;line-height:1.15}.executive-risks{display:grid;grid-template-columns:1fr 1fr;gap:1mm 2mm;margin:2mm 0}.executive-page .risk-pill{display:block;padding:.8mm 1.4mm;border-radius:1.5mm}.executive-page .risk-pill strong{color:var(--mint);font-size:6pt;line-height:1.15}.detail-page .evidence-layout{height:78mm}.detail-page .evidence-layout figure img{max-height:62mm}.detail-page .evidence-layout.with-image:has(.structured-list) figure img{max-height:46mm}
